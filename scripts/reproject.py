...
dask.config.set({'temporary_directory': 'D:/temp/'})
import os
...
@hydra.main(
    version_base="1.2", config_path=root / "configs", config_name="reproject.yaml"
)
def main(cfg: DictConfig) -> None:
    ...
    with ProgressBar():
        ...
        for day in days:
            zarr_day_path = Path(f'F:/eumetsat/process/2020/{day.strftime("%Y-%m-%d")}.zarr')
